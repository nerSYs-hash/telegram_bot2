...
def test_callback_handler():
    ...
    for name, pattern in sql_checks.items():
        ...
        if count >= 2:  # Должно быть минимум 2 раза (текст + Excel)
            print(f"   ✅ {name} - найден {count} раз(а)")
        else:
    ...
    if all_text_ok and all_excel_ok and all_sql_ok:
        print("✅ ВСЕ 7 ПАРАМЕТРОВ ДОБАВЛЕНЫ ПРАВИЛЬНО!")
        print("✅ Текстовое сообщение: OK")
        print("✅ Excel: OK")
        print("✅ SQL запросы: OK")
        return True
    else:
        print("❌ ЕСТЬ ОШИБКИ!")
        if not all_text_ok:
...
if __name__ == '__main__':
    ...
    print("\n" + "="*70)
    if success:
        print("🎉 ТЕСТ ПРОЙДЕН! ФАЙЛ ГОТОВ К ИСПОЛЬЗОВАНИЮ!")
    else:
