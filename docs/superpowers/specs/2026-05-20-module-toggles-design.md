...
- **Шиппер (7.4):** не дублируем настройки Экономики; решаем точку
  ...
  Экономики, где Шиппер начисляет). **Плюс UX-долг к этому шагу:**
  переработать сам экран Шиппера — сейчас большие неоправданные секции,
  непонятно зачем. На экране Шиппера явно показать инфо-блок «Управление
  параметрами начислений — в разделе Экономика» с прямой кнопкой-ссылкой
  на соответствующую секцию Экономики (`navigateTo('economy')` + якорь на
  блок Шиппера).

**Правило для всех будущих модулей** (запомнить в `feedback_*`):
1. Новый подтумблер добавляется ТОЛЬКО там, где есть осмысленное
   независимое поведение; дублирование с существующими настройками —
   запрещено.
2. Если функционал модуля **разделён** (UI модуля здесь, параметры — в
   другом разделе): на экране модуля обязательна прямая cross-link кнопка
   «Управление параметрами — в разделе X» с навигацией на конкретный
   якорь. Не оставлять пользователя гадать, где лежат настройки.
