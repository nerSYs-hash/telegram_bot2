...
>
> **Текущий процент готовности: ~95%** (28.05 Этап A ✅ + Этап C ✅ + Этап D ✅ + Stats per-ws ✅ + Экономика real-time ✅ + Этап B B1-B4 ✅ + CI guard ruff ✅).
...
## 📌 Прогресс Этапа E (V1.17.0R1-...)

Этап E фазирован (брейншторм 29.05, эталон — ChatKeeper):

| Фаза | Что | Статус |
|---|---|---|
| **E1** | Упрощённое подключение группы: авто-`main`, ноль выбора в чате (убрана портянка + утечка приватности), гейт регистрации убран, самоудаляемое приветствие + DM владельцу | ✅ V1.17.0R1 (за флагом `CONNECT_FLOW_V2`) |
| E1-флип | Включить `CONNECT_FLOW_V2`+`LOGIN_URL_BUTTON` на проде + финал текстов/ссылок + FAQ | 🔜 приёмка (с T9) |
| **E2** | Центр подключений на сайте: слоты ролей, диплинк `?startchannel` (механизм A) для журнал-канала/админ-чата + список «безролевых» чатов | 🔜 отдельная спека |
| **E3** | Онбординг-туториал нового владельца на сайте | 🔜 отдельная спека |

Спека/план E1: `docs/superpowers/specs/2026-05-29-stage-e1-onboarding-design.md`,
`docs/superpowers/plans/2026-05-29-stage-e1-onboarding.md`.
...
| ~~**Экономика real-time**~~ | ~~`get_dynamic_economy_config` хардкодит `_DEFAULT_WS_ID=1`~~ | ✅ V1.17.0N1 — workspace_id явный, в mining_logic пробрасывается из chat_id сообщения |

## ❌ Не работает (нарушает изоляцию)

| Аспект | Проблема | Куда фиксить |
|---|---|---|
| ~~**`db_friend.py` / `pulse_bot.db`**~~ | ~~Вторая БД без `workspace_id`~~ | ✅ V1.17.0O1-O5 — все 11 таблиц мигрированы, 8 функций ws-aware, callsites пробрасывают ws_id, регистрация per-ws через resolve_user_primary_workspace |
...
| ~~Регистрация (анкета)~~ | ~~`CHAT_ID` хардкод в `registration.py`~~ | ✅ V1.17.0O5 — _user_main_chat через resolve_user_primary_workspace, fallback на CHAT_ID |
