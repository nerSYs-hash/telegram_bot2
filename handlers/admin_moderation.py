"""
Модерация заявок на вступление и админ-действия над участниками:
одобрение/отклонение анкет, мут, бан, чёрный список, восстановление досье.
"""
...
async def send_admin_panel(bot, chat_id: int, is_owner: bool = False, thread_id: int = None):
    ...
    if is_owner:
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("💎 VIP BBS", callback_data="bbs_vip_root"),
...
async def handle_panel_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ─── ДОБАВИТЬ В ЧС: сначала ждём ID, потом причину ───
        ...
        try:
            ...
            from config import CHAT_ID as _CHAT_ID
            main_db = context.bot_data.get('db')
            try:
                _chat_obj = await context.bot.get_chat(_CHAT_ID)
            except Exception:
                _chat_obj = None
            await log_blacklist(
                context.bot, main_db, target_id, update.effective_user.id, True,
                reason=text, admin_user=update.effective_user, chat=_chat_obj,
            )
    ...
    # ─── УДАЛИТЬ ИЗ ЧС ───
        ...
        try:
            ...
            await log_blacklist(
                ...
                admin_user=update.effective_user, chat=_chat_obj,
            )
        except Exception as je:
