...
"""
Статистика — экспорт, топы, лидерборды — вынесены из callback_handler.py

Data Integrity: все числовые вычисления (индексы, коэффициенты, суммы)
выполняются через Decimal для исключения ошибок округления.
format_number() используется ТОЛЬКО для отображения в Telegram-сообщениях.
