...
SHOP_CATALOG = {
    'ego': {
        ...
        'items': {
            ...
            'title': {'name': '🏷️ Кастомный Титул (Лычка)', 'price': 1, 'desc': 'Бот выдает тебе тег к нику (до 16 букв). Например: [Спонсор], [Daddy]. (7 дней)'},
...
async def buy_shop_item(query, context, db, user_id, cat_id, item_id):
    ...
    # ════════════════════════════════════════════════════════════════════════════════
    # 🏷️ КАСТОМНЫЙ ТИТУЛ (title)
    # ════════════════════════════════════════════════════════════════════════════════
    if item_id == 'title':
        # Запрашиваем у пользователя текст титула
        context.user_data['awaiting'] = f'title_text_{user_id}'
        
        success_text = (
            f"✅ <b>ТИТУЛ ГОТОВ!</b>\n\n"
            f"Напишите боту в ЛС текст вашего титула (до 16 букв).\n\n"
            f"<i>Примеры: [Спонсор], [Daddy], [Босс], [VIP]</i>\n\n"
            f"<b>Срок действия: 7 дней</b>\n\n"
            f"После отправки текста бот автоматически:\n"
            f"• Выдаст вам админские права\n"
            f"• Повесит приписку к нику в интерфейсе\n"
            f"• Сделает вас модератором чата! 🏷️"
        )
        await query.edit_message_text(
            success_text,
        ...
        # Добавить минимальную запись в БД
        import time as time_module
        ...
        db.cursor.execute(
            ...
            (expires_at, '[ТИТУЛ]', user_id, 'title', 'active')
        )
        db.conn.commit()
        return
